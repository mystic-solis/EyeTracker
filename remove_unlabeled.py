...
def main(**kwargs):
    ...
    del_coef = float(kwargs.get("mode").split(":")[-1])
    ...
    annotations_jsons = glob(os.path.join(annotations_path, "**", "*.json"), recursive=True)
    ...
    for json_path in annotations_jsons:
        ...
        num_to_delete = int(non_annot_len - annot_len*del_coef)
        if verbose:
